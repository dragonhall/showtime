...
class Channel < ApplicationRecord
  ...
  def permit_fulladmins
    unless group_ids.include?(Group.pluck(:id).first) then
      groups << Group.where(name: 'FullAdmin').first
    end
  end

  def trailer_presence
    errors.add(:trailer_before) unless Video.where(id: trailer_before_id).any?
    errors.add(:trailer_after) unless Video.where(id: trailer_after_id).any?
  end
end
